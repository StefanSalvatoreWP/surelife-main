...
@section('styles')
    <link rel="stylesheet" href="{{ asset('css/contractbatch.css') }}?v={{ time() }}">
@endsection
...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-xl border-2 border-green-300 p-6 mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-green-800 mb-2">Contract Batch</h1>
                    <p class="text-green-600 text-sm">Manage contract by batch codes and assigned staffs</p>
                </div>
        ...
        <!-- Action Button -->
        <div class="mb-6">
            <a href="/contractbatch-create" class="inline-flex items-center px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transform hover:scale-105 transition duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <h3 class="text-lg font-semibold text-gray-800">Contract Batch List</h3>
                <div class="flex flex-col sm:flex-row sm:items-center gap-3 w-full lg:w-auto">
                    <label for="branchFilter" class="text-sm font-medium text-gray-600">Filter by Branch</label>
                    <select id="branchFilter" class="w-full sm:w-60 rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-700 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        <option value="">All Branches</option>
                        @foreach($branches ?? [] as $branch)
                            <option value="{{ $branch->Id ?? $branch->id }}">{{ $branch->BranchName ?? $branch->branchname }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <!-- MODAL DELETE -->
        <div class="modal fade" id="contractbatchDeleteModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true" style="display: none;">
                    ...
                    <div class="modal-body p-6">
                        <div class="flex items-start mb-4">
                            ...
                            <div class="ml-4 flex-1">
                                <p class="text-gray-800 font-semibold mb-2">Delete selected batch? <span class="text-red-600 font-bold" id="contractbatchToDelete"></span></p>
                                <p class="text-gray-600 text-sm">This action cannot be undone. The contract batch will be permanently removed from the system.</p>
                            </div>
                        </div>
                    </div>
    ...
    <script src="{{ asset('js/contractbatch.js') }}"></script>
@endsection