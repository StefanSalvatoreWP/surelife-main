    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-xl border-2 border-green-200 p-6 mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-green-800 mb-2">Update Bank</h1>
                    <p class="text-green-600 text-sm">Update selected bank for your accounts</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="p-6">
                <form action="/submit-bank-update/{{ $banks->bankid }}?account_number={{ $banks->AccountNumber }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            @php
                                $selectedOption = old('bankname', $banks->BankName)
                            @endphp
                            <label for="bankName" class="block text-sm font-semibold text-gray-700 mb-2">Bank Name</label>
                            <input type="text" class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" id="bankName" name="bankname" maxlength="30" value="{{ $selectedOption }}" readonly />
                            @error('bankname')
                        ...
                        <div>
                            @php
                                $selectedOption = old('accountno', $banks->AccountNumber)
                            @endphp
                            <label for="accountNo" class="block text-sm font-semibold text-gray-700 mb-2">Account No.</label>
                            <input type="text" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" id="accountNo" name="accountno" maxlength="30" value="{{ $selectedOption }}" />
                            @error('accountno')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
