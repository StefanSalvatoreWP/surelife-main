    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-xl border-2 border-green-200 p-6 mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-green-800 mb-2">Update Menu</h1>
                    <p class="text-green-600 text-sm">Update selected menu to display on the system.</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="p-8">
                <form action="/submit-menu-update/{{ $menu->Id }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            @php
                                $prevMenuName = old('menuname', $menu->MenuItem);
                            @endphp
                        ...
                        <div>
                            @php
                                $prevMenuLevel = old('menurolelevel', $menu->RoleLevel);
                            @endphp
                            <label for="menuRoleLevel" class="block text-sm font-semibold text-gray-700 mb-2">Level</label>
                            <input type="number" 
                                   class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 @error('menurolelevel') border-red-500 @enderror" 
                                   id="menuRoleLevel" 
                                   name="menurolelevel" 
                                   maxlength="30" 
                                   value="{{ $prevMenuLevel }}"
