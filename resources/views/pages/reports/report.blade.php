    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6 rounded-lg shadow-sm">
            <div class="flex items-center">
                ...
                <p class="text-blue-700 font-medium">Obtain reports from the desired branch based on the selected report
                    type.</p>
            </div>
        </div>
                ...
                <form action="{{ route('reports.daily') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="reportTypes"
                            ...
                            <option value="0" hidden @if(!old('dailyreporttype')) selected @endif>-- Select Report --
                            </option>
                            @foreach($report_types as $reportType)
                                <option value="{{ $reportType }}" @if(old('dailyreporttype')) selected @endif>{{ $reportType }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="pt-2 flex flex-col sm:flex-row gap-3">
                        <button formaction="{{ route('reports.daily') }}" formmethod="POST"
                            class="flex-1 px-8 py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                            type="submit">
                        ...
                        <button formaction="{{ route('reports.daily.pdf') }}" formmethod="POST"
                            class="flex-1 px-8 py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
                            type="submit">
                            <span class="flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd"
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden mt-6">
            ...
            <div class="p-6">
                <form action="{{ route('reports.status') }}" method="POST" class="space-y-4">
                    @csrf

                    {{-- Report Type --}}
                    <div>
                        <label for="statusReportType" class="block text-sm font-medium text-gray-700 mb-2">
                            <svg class="w-4 h-4 inline mr-1 text-gray-500" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" />
                                <path fill-rule="evenodd"
                                    d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z"
                        ...
                        <select id="statusReportType" name="statusreporttype"
                            ...
                            <option value="0" hidden @if(!old('statusreporttype')) selected @endif>-- Select Report Type --
                            </option>
                            @foreach($status_report_types as $type)
                                <option value="{{ $type }}" @if(old('statusreporttype') == $type) selected @endif>
                                    {{ $type }}
                                </option>
                            @endforeach
                        </select>
                        @error('statusreporttype')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    {{-- Branch --}}
                    <div>
                        <label for="statusBranch" class="block text-sm font-medium text-gray-700 mb-2">
                            <svg class="w-4 h-4 inline mr-1 text-gray-500" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                                    d="M4 4a2 2 0 012-2h8a2 2 0 012 2v12a1 1 0 110 2h-3a1 1 0 01-1-1v-2a1 1 0 00-1-1H9a1 1 0 00-1 1v2a1 1 0 01-1 1H4a1 1 0 110-2V4zm3 1h2v2H7V5zm2 4H7v2h2V9zm2-4h2v2h-2V5zm2 4h-2v2h2V9z"
                                    clip-rule="evenodd" />
                        ...
                        <select id="statusBranch" name="statusbranch"
                            class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition duration-200">
                            <option value="0" hidden @if(!old('statusbranch')) selected @endif>-- Select Branch --</option>
                            @foreach($branch_list as $branch)
                                <option value="{{ $branch->Id }}" @if(old('statusbranch') == $branch->Id) selected @endif>
                                    {{ $branch->BranchName }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    {{-- Optional Date Range (shown for Transfer & Completed) --}}
                    ...
                    {{-- Export Buttons --}}
                    <div class="pt-2 flex flex-col sm:flex-row gap-3">
                        <button formaction="{{ route('reports.status') }}" formmethod="POST"
                            class="flex-1 px-8 py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                            type="submit">
                            <span class="flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <button formaction="{{ route('reports.status.pdf') }}" formmethod="POST"
                            class="flex-1 px-8 py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
                            type="submit">
                            <span class="flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
