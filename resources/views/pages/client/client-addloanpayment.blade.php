        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-6">
            <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-indigo-50">
                <div class="flex items-center justify-between">
                    ...
                    @endphp
                    <a href="/client-view/{{ $clients->Id }}{{ $statusQuery }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-gradient-to-br from-amber-50 to-amber-100 rounded-lg p-4 border border-amber-200">
                ...
                <p class="text-xl font-bold text-amber-900">
                    {{ $loanRequestData->term_months ?? $loanRequestData->TermMonths ?? 12 }} months</p>
            </div>
        </div>

        <!-- Advance Payment Preview (Dynamic) -->
        <div id="advancePaymentPreview"
            class="hidden bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg p-4 border border-indigo-200 mb-6">
            ...
            <form action="/client-submit-loanpayment/{{ $clients->Id }}" method="POST" class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div class="relative">
                        ...
                        <p class="text-xs text-gray-500 mb-1 h-4">Minimum: ₱ {{ number_format($minPayableAmount, 2) }} |
                            Max: ₱ {{ number_format($loanBalance, 2) }}</p>
                        <input type="number"
                            ...
                            id="paymentAmount" name="paymentamount" min="{{ $minPayableAmount }}" max="{{ $loanBalance }}"
                            step="0.01" value="{{ old('paymentamount') ?: $minPayableAmount }}"
                            placeholder="Enter payment amount" autocomplete="off" required />
                        <div id="paymentSuggestions"
                            class="hidden absolute top-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
                            @foreach($amounts as $amount)
                                <div class="px-4 py-2 hover:bg-purple-50 cursor-pointer text-sm" data-value="{{ $amount }}">₱
                                    {{ number_format($amount, 2) }}</div>
                            @endforeach
                        </div>
                        <p id="paymentAmountError" class="text-red-500 text-sm mt-1 hidden">Amount must be at least ₱
                            {{ number_format($loanMonthlyAmount, 2) }}</p>
                    ...
                    <div>
                        <label for="orSeriesCode" class="block text-sm font-medium text-gray-700 mb-2">O.R Series Code <span
                        ...
                        <div class="relative">
                            <input type="text"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition"
                                id="orSeriesCode" name="orseriescode" maxlength="30" value="{{ old('orseriescode') }}"
                                placeholder="Optional - Leave empty if no O.R" autocomplete="off" />
                            <div id="orSeriesCodeDropdown"
                                class="hidden absolute top-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
                                <!-- Dropdown options will be populated here -->
                            </div>
                        </div>
                        <div id="orSeriesCodeLoading" class="hidden text-sm text-gray-500 mt-1">Loading O.R series codes...
                        </div>
                    ...
                    <!-- O.R No. (Optional) -->
                    <div>
                        <label for="orNo" class="block text-sm font-medium text-gray-700 mb-2">O.R No. <span
                                class="text-gray-400 text-xs">(Optional)</span></label>
                        <p class="text-xs text-gray-500 mb-1">Leave empty if no O.R needed</p>
                        <div class="relative">
                            <input type="text"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition"
                                id="orNo" name="orno" maxlength="30" value="{{ old('orno') }}"
                                placeholder="Optional - Leave empty if no O.R" autocomplete="off" />
                            <div id="orNoDropdown"
                                class="hidden absolute top-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
                                <!-- Dropdown options will be populated here -->
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="date"
                            class="w-full md:w-1/2 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition"
                            id="paymentDate" name="paymentdate" value="{{ old('paymentdate') ?: date('Y-m-d') }}" />
