...
$(document).ready(function() {
        ...
        columns: [
            ...
            { data: 'Province', name: 'tblprovince.Province' },
            {
                ...
                render: function (data, type, row) {
                    var updateLink = '<a href="/city-update/' + data.Id + '" class="action-btn action-btn-update">' +
                    ...
                    var deleteLink = '<a href="#" data-city-id="' + data.Id + '" data-city-name="' + data.City + '" class="action-btn action-btn-delete">' +
                        '<svg class="action-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">' +
    ...
    $(document).on('click', '.action-btn-delete', function(e) {
        ...
        $('#cityToDelete').text(cityName);
        $('#confirmDelete').off('click');
        
        $('#confirmDelete').on('click', function() {
            let deleteForm = $('#deleteForm');
            deleteForm.attr('action', '/submit-city-delete/' + cityId);
            deleteForm.submit();
        });
        
        let modal = $('#cityDeleteModal');
    ...
    $(document).on('click', '[data-bs-dismiss="modal"]', function() {
        $('#cityDeleteModal').removeClass('show').css('display', 'none');
    });
});