...
$(document).ready(function() {
        ...
        columns: [
            ...
            { data: 'Province', name: 'Province' },
            {
                ...
                render: function (data, type, row) {
                    var updateLink = '<a href="/province-update/' + data.Id + '" class="action-btn action-btn-update">' +
                    ...
                    var deleteLink = '<a href="#" data-province-id="' + data.Id + '" data-province-name="' + data.Province + '" class="action-btn action-btn-delete">' +
                        '<svg class="action-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">' +
    ...
    $(document).on('click', '.action-btn-delete', function(e) {
        ...
        let provinceName = $(this).data('province-name');
        
        $('#provinceToDelete').text(provinceName);
        $('#confirmDelete').off('click');
        
        $('#confirmDelete').on('click', function() {
            let deleteForm = $('#deleteForm');
            deleteForm.attr('action', '/submit-province-delete/' + provinceId);
            deleteForm.submit();
        });
        
        let modal = $('#provinceDeleteModal');
    ...
    $(document).on('click', '[data-bs-dismiss="modal"]', function() {
        $('#provinceDeleteModal').removeClass('show').css('display', 'none');
    });
});